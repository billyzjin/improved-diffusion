...
# Aggregates results from parallel MNIST evaluation jobs into a summary file.
...
if [ -n "$1" ]; then
    ...
    EVAL_DIR=$(ls -1d /project_gpfs/bata0/bjin0/mnist_evaluation_parallel_* 2>/dev/null | sort | tail -1)
    if [ -z "$EVAL_DIR" ]; then
        echo "ERROR: No 'mnist_evaluation_parallel_*' directory was found in /project_gpfs/bata0/bjin0"
        exit 1
    fi
...
echo "=========================================="
echo "CREATING MNIST RESULTS SUMMARY"
echo "Aggregating results from: $EVAL_DIR"
echo "=========================================="

echo "MNIST MODEL EVALUATION RESULTS" > "$RESULTS_FILE"
echo "==============================" >> "$RESULTS_FILE"
