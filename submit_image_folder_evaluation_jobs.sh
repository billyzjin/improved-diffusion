...
if [ ! -f "$SUBMISSION_TSV" ]; then
    printf "submitted_at\tstatus\tjob_id\tdataset\tschedule_name\tobjective\trun_name\teval_name\tmodel_path\teval_dir\tnll_num_samples\tfid_num_samples\tforce\tskip_nll\tskip_fid\n" > "$SUBMISSION_TSV"
fi
...
while IFS=$'\t' read -r submitted_at status job_id dataset schedule_name objective run_name train_dir image_size train_steps time_limit dependency prep_job_id logdir; do
    ...
    export_arg="ALL,DATASET=${DATASET},EVAL_NAME=${eval_name},RUN_NAME=${run_name},SCHEDULE_NAME=${schedule_name},OBJECTIVE=${objective},MODEL_PATH=${model_path},PARENT_EVAL_DIR=${PARENT_EVAL_DIR},TRAIN_DIR=${TRAIN_DIR},TEST_DIR=${TEST_DIR},STATS_FILE=${STATS_FILE},IMAGE_SIZE=${IMAGE_SIZE},SAMPLE_BATCH_SIZE=${SAMPLE_BATCH_SIZE},FORCE=${FORCE},SKIP_NLL=${SKIP_NLL},SKIP_FID=${SKIP_FID}"

    echo "SUBMIT eval $eval_name model=$model_path"
    if [ "$DRY_RUN" != "1" ]; then
        sbatch_output=$(
    ...
    printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
        "$(date -Is)" "$eval_status" "$eval_job_id" "$DATASET" "$schedule_name" "$objective" "$run_name" "$eval_name" "$model_path" "$PARENT_EVAL_DIR/$eval_name" "10000" "50000" "$FORCE" "$SKIP_NLL" "$SKIP_FID" >> "$SUBMISSION_TSV"
