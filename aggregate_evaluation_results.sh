...
if [ -n "$1" ]; then
    ...
    EVAL_DIR=$(ls -1d /project_gpfs/bata0/bjin0/evaluation_parallel_* 2>/dev/null | sort | tail -1)
    
    if [ -z "$EVAL_DIR" ]; then
        echo "ERROR: No 'evaluation_parallel_*' directory was found in /project_gpfs/bata0/bjin0"
        exit 1
    fi
