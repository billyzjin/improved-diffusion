...
Before stating the main identity, we record a general calculus lemma that will be used in the proof. It says that when we differentiate the expectation of a \(u\)-dependent function along the Gaussian-convolution path \(Y_u\), we pick up two contributions: the explicit \(u\)-derivative of the integrand, and a Laplacian term coming from the heat equation for the density.
...
\]
Writing \(\Delta:=\sum_{i=1}^d\partial_{ii}\) for the Laplacian, we apply \Cref{lem:heat_expectation_identity} with
\[
f_u(y)=\log\det A_u(y),
\]
which gives
...
\]
where the middle equality is the same integration-by-parts argument used in the proof of \Cref{lem:heat_expectation_identity}.
