...
HYBRID_VB_WEIGHT_GRID=0
if [ -n "${HYBRID_VB_WEIGHTS+x}" ]; then
    HYBRID_VB_WEIGHT_GRID=1
else
...
case "$DATASET" in
    cifar10)
        PREP_SLURM=${PREP_SLURM:-prepare_cifar10.slurm}
        DATA_ROOT=${DATA_ROOT:-/home/bjin0/improved-diffusion}
        TRAIN_DIR=${TRAIN_DIR:-$DATA_ROOT/cifar_train}
        DATASET_READY_MARKER=${DATASET_READY_MARKER:-$TRAIN_DIR}
    ...
    lsun_bedroom64)
        ...
        ;;
    lsun_church64)
        PREP_SLURM=${PREP_SLURM:-prepare_lsun_church64.slurm}
...
if [ -n "${DATASET_READY_MARKER:-}" ] && [ "$FORCE_PREP" != "1" ]; then
    if [ -f "$DATASET_READY_MARKER" ] || [ -d "$DATASET_READY_MARKER" ]; then
        dataset_ready=1
...
if [ "$HYBRID_VB_WEIGHT_GRID" = "1" ]; then
...
for schedule_name in "${schedules[@]}"; do
    ...
    for objective in "${objectives[@]}"; do
        ...
        if [ "$objective" = "hybrid" ]; then
            objective_weights=("${hybrid_vb_weights[@]}")
        fi
        for hybrid_vb_weight in "${objective_weights[@]}"; do
            run_name="${schedule_name}_${objective}"
            job_name="${DATASET}_${short_schedule}_${objective}"
            if [ "$objective" = "hybrid" ] && [ "$HYBRID_VB_WEIGHT_GRID" = "1" ]; then
                slug=$(weight_slug "$hybrid_vb_weight")
