...
if [ -n "$1" ]; then
    ...
    EVAL_DIR=$(find /project_gpfs/bjin0 -name "fashion_evaluation_parallel_*" -type d 2>/dev/null | sort | tail -1)
    if [ -z "$EVAL_DIR" ]; then
        echo "ERROR: No 'fashion_evaluation_parallel_*' directory was found in /project_gpfs/bjin0"
        exit 1
    fi
