...
# Aggregates results from parallel ImageNet-64 evaluation jobs into a summary file.
...
if [ -n "$1" ]; then
  ...
  # These eval dirs live at the top-level under /project_gpfs/bata0/bjin0.
  EVAL_DIR=$(ls -1d /project_gpfs/bata0/bjin0/imagenet64_evaluation_parallel_* 2>/dev/null | sort | tail -1)
  if [ -z "$EVAL_DIR" ]; then
    echo "ERROR: No 'imagenet64_evaluation_parallel_*' directory was found in /project_gpfs/bata0/bjin0"
    exit 1
  fi
...
echo "=========================================="
echo "CREATING IMAGENET-64 RESULTS SUMMARY"
echo "Aggregating results from: $EVAL_DIR"
echo "=========================================="

echo "IMAGENET-64 MODEL EVALUATION RESULTS" > "$RESULTS_FILE"
echo "====================================" >> "$RESULTS_FILE"
...
echo "==============================================" >> "$RESULTS_FILE"
for exp_dir in "$EVAL_DIR"/imagenet64_*; do
  if [ -d "$exp_dir" ] && [ -f "$exp_dir/nll_results.txt" ]; then
    ...
    if [ -n "$nll_score" ]; then
      printf "%-28s: %s bits/dimension\n" "$exp_name" "$nll_score" >> "$RESULTS_FILE"
    else
      printf "%-28s: ERROR extracting NLL\n" "$exp_name" >> "$RESULTS_FILE"
    fi
  fi
done
  ...
  if [ -d "$exp_dir" ]; then
    ...
    if [ -f "$exp_dir/fid_results.txt" ]; then
      ...
      if [ -n "$fid_score" ]; then
        ...
        printf "%-28s: ERROR extracting FID\n" "$exp_name" >> "$RESULTS_FILE"
      fi
    else
      printf "%-28s: FID not found\n" "$exp_name" >> "$RESULTS_FILE"
    fi
  fi
done
  ...
  if [ -d "$exp_dir" ]; then
    ...
    if [ -f "$exp_dir/tv_results.txt" ]; then
      ...
      if [ -n "$tv_score" ]; then
        ...
        printf "%-28s: ERROR extracting TV\n" "$exp_name" >> "$RESULTS_FILE"
      fi
    else
      printf "%-28s: TV not found\n" "$exp_name" >> "$RESULTS_FILE"
    fi
  fi
done
...
for exp_dir in "$EVAL_DIR"/imagenet64_*; do
  if [ -d "$exp_dir" ]; then
    exp_name=$(basename "$exp_dir")
    if [ -f "$exp_dir/samples.npz" ]; then
      printf "%-28s: Samples generated successfully\n" "$exp_name" >> "$RESULTS_FILE"
    else
      printf "%-28s: No samples found\n" "$exp_name" >> "$RESULTS_FILE"
    fi
  fi
done
